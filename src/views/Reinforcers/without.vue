       ...
       <tbody>
           ...
           <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
               <th  scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                   <div v-for="pair,index in arr.length-1" >
                       {{ index+1 }}
 ...
 export default {
   ...
   methods: {
    selectItem(item, index) {
        this.pair.result[index]=item
        console.log(this.pair.result)
          if (index === this.arr.length - 1) {  // Ensure only selecting from the latest array
            const newArray = this.arr[index].filter(i => i !== item);
            this.arr.push(newArray);
          }
        },
