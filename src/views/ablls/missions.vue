             ...
             <div v-for="data,index in users" class="grid grid-cols-3 m-[1%] shadow-md p-[2%] bg-gray-200 rounded-md">
               <div class="col-span-2">
                <div class="flex">
                   ...
                   <div style="overflow: hidden !important;" class="flex max-w-full">
                      <h3 class="text-base font-bold " style="word-wrap: break-word;" >{{ $t("degree") }}</h3>
                      <p >: {{ data.degree }}</p>
                   </div>
                   <div style="overflow: hidden !important;" class="flex max-w-full">
                      <h3 class="text-base font-bold " style="word-wrap: break-word;" >{{ $t("key") }}</h3>
                      <p >: {{ data.key }}</p>
                   </div>
               </div>
               <div class="flex">
...
onBeforeMount(() => {
 ...
 const fetchData= ()=>{
  ...
  axios.get("/api/able-mission").then((res)=>{
    loading.value= false
...
const edit=(id)=>{
  router.push({name:'mission-update',params:{'id':id}})
}
...
const editescrud=()=>{
    axios
    .put(`/api/able-mission/${confir_id.value}`,levels.value)
    .then((res) => {
...
const createcrude=()=>{
    axios
    .post('/api/able-mission',levels.value)
    .then((res) => {
      console.log(res.data)
      fetchData()
...
const deleteAction = () => {
  axios
    .delete(`/api/able-mission/${confir_id.value}`)
    .then((res) => {
      console.log(res.data)
