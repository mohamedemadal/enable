...
onBeforeMount(() => {
 ...
 const fetchData= ()=>{
  ...
  axios.get("/api/barrier-question").then((res)=>{
    loading.value= false
...
const edit=(id)=>{
    axios.get(`/api/barrier-question/${id}`).then((res)=>{
    loading.value= false
...
const editescrud=()=>{
    axios
    .post(`/api/barrier-question/${confir_id.value}`,levels.value)
    .then((res) => {
...
const createcrude=()=>{
    axios
    .post('/api/barrier-question',levels.value)
    .then((res) => {
      console.log(res.data)
      fetchData()
      createdialog.value=!(createdialog.value)
...
const deleteAction = () => {
  axios
    .delete(`/api/barrier-question/${confir_id.value}`)
    .then((res) => {
      console.log(res.data)
      ...
      <div style="" class="shadow-xl ">
        ...
        <Dialog v-model:visible="createdialog" :style="{ width: '450px' }" :header='$t("submit")' :modal="true">
            ...
            <div class="flex flex-column gap-2">
                  ...
                  <Dropdown required id="pv_id_1" style="direction: ltr !important;" v-model="levels.subtest_id"  option-value="id" filter :options="all_levels" optionLabel="title" :placeholder='$t("Subtest_id")' class="w-full bg-[#f7f5f5] [&>div>div>span]:bg-black md:w-14rem " />
